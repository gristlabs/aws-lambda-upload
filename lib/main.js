 ...
 *
 * TODO
 * proposing revamping the interface and functionality:
 * - want output to be any one of:
 *   zip file
 *   path to new s3 object (named by md5 hash)
 *   lambda update
 * - want additional option or separate tool to:
 *   take CFN template with .CodeUri that's a path to a file, must be entry file (js, not zip),
 *   colllect and replace with s3 URL, and print out the resulting template. This is similar to
 *   cloudformation package command, except collects dependencies.
 *   (basically as long as doesn't start with s3:// or https://s3.amazonaws.com/
 *   At the end, WILL start with s3://
 *
 * --lambda <name> <path>
 * --zip <dest> <path>
 * --s3, -s <path> -- prints to stdout
 * --cfn <path> interpreted as template path, prints new template to stdout.
 */
function main() {
  commander
  ...
  .option('--paths <paths>', 'Colon-delimited list of extra directories to search for modules',
    val => val.split(':'))
  .option('--tsconfig <path>', 'If given, support TypeScript, and use the given tsconfig.json')
